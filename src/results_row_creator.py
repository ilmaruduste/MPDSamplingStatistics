...
class ResultsRowCreator:
    ...
    def get_grouped_orig_data(self, group_name, group_value_array):
        grouped_data = self.original_data.copy()
        
        # This is done so multiple groups could be used
        for i, name in enumerate(group_name):
            grouped_data = grouped_data[grouped_data[name] == group_value_array[i]]

        return grouped_data

    def get_grouped_comp_data(self, group_name, group_value_array):
    ...
    def get_grouped_joined_data(self, group_name, group_value_array):
...
class DomInbResultsRowCreator(ResultsRowCreator):
    ...
    def get_row_of_statistics_with_group(self, table_name, data_type, indicator_name, group_name, group_value_array):
        ...
        combination_coverage_array = [self.calculate_combination_coverage(group_name, group_value_combination) for group_value_combination in group_value_product_array]
        ks_test_array = [self.calculate_ks_test(indicator_name, group_name, group_value_combination) for group_value_combination in group_value_product_array]
        indicator_mean_array = [self.calculate_original_indicator_mean(indicator_name, group_name, group_value_combination) for group_value_combination in group_value_product_array]
        indicator_mad_array = [self.calculate_original_indicator_mad(indicator_name, group_name, group_value_combination) for group_value_combination in group_value_product_array]
        ape_metrics_array = np.array([self.calculate_absolute_percentage_error_metrics(indicator_name, group_name, group_value_combination) for group_value_combination in group_value_product_array]).T
        ale_metrics_array = np.array([self.calculate_absolute_logarithmic_error_metrics(indicator_name, group_name, group_value_combination) for group_value_combination in group_value_product_array]).T
